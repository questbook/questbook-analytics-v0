...
const insertNewGrantApplications = async (sql: mysql.Connection, chainId: number, grantApplications: any[]) => {
  ...
  const [rows, fields] = await sql.execute(`insert into grantApplications (applicationId, applicantAddress, createdAt, updatedAt, isAccepted, grantId, chainId, isPending, workspaceId) values ${insertString}`)
